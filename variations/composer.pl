...
sub init_composer {
	...
	\&repeat_recent_literal,	\&repeat_recent_literal,
	...
	\&harmonize_last_phrase,	\&harmonize_last_phrase, \&harmonize_last_phrase,
					\&harmonize_last_phrase, \&harmonize_last_phrase,
    );
...
sub do_special_event {
    ...
    &{ $special_events[ int(rand(@special_events)) ] } ;
}

sub do_development {
    print STDERR "do_development\n" if $DEBUG;

    &{ $development_table[ int(rand(@development_table)) ] } ;
}
...
sub compose_phrase {
    ...
    $Phrases{ $ID.$Measures }	= &random_measures if $ID;
...
#    }
	...
	if (&random($KeyChange->{ $_ })) {
	    $Phrases{ $ID.$_ } =	&{ $vary->{ $_ } } (\%Phrases, $ID-1);
	} else {
...
#	$saveid = $ID;
	...
	&{ $composer_array[ int(rand(@composer_array)) ] };
...
sub debug_stuff {
    ...
    if ($ID != $saveid) {
	$a = $Phrases{ $saveid . $Pitches };
	print STDERR "create_score: id $saveid has pitch list:" ;
	&print_array($a, $a->[0], 'create_score');
    }

    print STDERR "BEGIN PHRASES\n" ;
    for ($j=0; $j<$ID; $j++) {
	print STDERR "PHRASES: printing phrase $j \n";
	for (@NoteTypes) {
	    $a = $Phrases{ $j.$_ };
	    &print_array($a, $a->[0], 'PHRASES');
	}
	for (@NotNoteTypes) {
	    print STDERR "print value in PHRASES: $_ value ", $Phrases{ $j.$_ }, "\n";
	}
    }
    print STDERR "END PHRASES\n" ;
}
